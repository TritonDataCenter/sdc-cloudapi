...
function getCreateOptions(req) {
    ...
    if (params.networks && !/6\.5/.test(req.getVersion())) {
        opts.networks = params.networks;

        if (!Array.isArray(opts.networks)) {
            opts.networks = [];
        }
    } else {
        ...
        if (pkgNetworks) {
            ...
            if (defaultNetworks.indexOf(EXTERNAL_NIC_TAG) !== -1) {
                if (req.external_nets.length) {
                    opts.networks.push(req.external_nets[0]);
                }
            }

            if (defaultNetworks.indexOf(INTERNAL_NIC_TAG) !== -1) {
                if (req.internal_nets.length) {
                    opts.networks.push(req.internal_nets[0]);
                }
            }
        }
    }
...
function loadMachine(req, res, next) {
    ...
    if (!req.params.machine || !(/\/machines/.test(req.url))) {
        return next();
    }
    ...
    return vasync.pipeline({
        ...
        if (machine.role_tags && machine.role_tags.length) {
            req.machine['role-tag'] = machine.role_tags;
        }
        return next();
    });
}
...
function create(req, res, next) {
    ...
    return req.sdc.vmapi.createVm(opts, {
        ...
        if (err) {
            // PUBAPI-791: Add some extra information about validation errors
            // for some parameters (and intentionally hide others accepted
            // by vmapi, like server_uuid and similar):
            var body = err.body;
            var msgs = [];
