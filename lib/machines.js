...
function translate(machine, credentials, datasets) {
    ...
    if (credentials) {
        ...
        if (Object.keys(msg.metadata.credentials).length === 0) {
...
function loadMachine(req, res, next) {
    ...
    return req.sdc.vmapi.getVm({
        ...
        // If we are loading a single machine, we'll try to get the associated
        // image, instead of preloading every image to find our current one:
        if (!req.datasets) {
            ...
            if (req.params.image === null) {
                return next(new ResourceNotFoundError('Cannot find machine ' +
                        'image'));
            } else {
                return images.loadImage(req, function (err2, img) {
                    if (err) {
                        return next(err2);
                    }
                    req.dataset = img;
                    ...
                    req.datasets = [img];
                    req.machine = translate(machine, req.params.credentials,
                        req.datasets);
                    return next();
                });
            }
...
function create(req, res, next) {
    ...
    if (req.dataset.requirements && req.dataset.requirements.networks &&
            (req.dataset.requirements.networks.length > 2)) {
        max_networks = req.dataset.requirements.networks.length;
    }

    if (opts.networks.length > max_networks) {
