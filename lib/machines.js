...
function create(req, res, next) {
    ...
    // Finally, add role tags to the create params as final step:
    var role_tags = (req.headers['role-tag']) ?
        req.headers['role-tag'].split(',') : [];

    return membership.preloadGroups(req, role_tags, function (er, roles) {
        if (er) {
            req.log.error({
                err: er,
