...
function getCreateOptions(req) {
    ...
    // flexible_disk is only supported on bhyve but operators should be able
    // to create a single package specification that works with any brand
    // type.
    if (pkg.flexible_disk && brand !== 'bhyve') {
        if (pkg.flexible_disk) {
            delete pkg.flexible_disk;
        }
    }
    ...
    if (['bhyve', 'kvm'].indexOf(brand) !== -1) {
        ...
        if (pkg.flexible_disk) {
            ...
            if (disks) {
                ...
                disks.forEach(function validateBlockSize(disk, i) {
                    if (disk.hasOwnProperty('block_size')) {
                        if (disk.hasOwnProperty('image_uuid')) {
                            throw new InvalidArgumentError(
                                'Cannot set block_size of disk with image_uuid'
                            );
                        }
                        if (!validRecordSize(disk.block_size)) {
                            throw new InvalidArgumentError(
        ...
        funcs: [
            function _getVm(_, cb) {
                req.sdc.vmapi.getVm({
                     ...
                     * https://github.com/TritonDataCenter/rfd/blob/master/rfd/0026/README.md
		     * for more details.
                     */
