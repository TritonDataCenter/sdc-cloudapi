    ...
    plugins.forEach(function (plugin) {
        if (plugin.enabled) {
            ...
            if (typeof (p[plugin.name].preProvision) === 'function') {
                preProvision.push(p[plugin.name].preProvision(cfg[plugin.name]));
                // server.use(plugins[plugin.name].preProvision(plugins_cfg[plugin.name]));
            }

            if (typeof (p[plugin.name].postProvision) === 'function') {
                postProvision.push(p[plugin.name].postProvision(cfg[plugin.name]));
            }
        }
    });
