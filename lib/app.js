...
function createUfdsClient(options, callback) {
    ...
    function reconnectOnTimeout() {
        options.log.info('UFDS client timeout, trying to reconnect');
        ufds.client = null;
        // We'll attempt to re-connect, which will set again ufds.client
        // if connection succeed and emit 'ready'. On failure, it will emit
        // 'error'. We'll keep retrying on error:
        ufds.once('error', function (err) {
            options.log.error({err: err}, 'UFDS error');
    ...
    ufds.on('timeout', reconnectOnTimeout);

    return ufds.once('ready', function () {
        options.log.debug('Successfully bound to UFDS');
        ufds.removeAllListeners('error');
        // Add new on('error') callback for reconnect:
        ufds.client.on('close', function (err) {
            options.log.error({err: err}, 'UFDS connection closed');
