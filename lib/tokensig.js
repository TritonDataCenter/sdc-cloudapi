...
module.exports = {
   ...
   *                   - headers: header names to require (default: date or
   *                               x-date).
   ...
   *                              in options.
   * @throws {ExpiredRequestError} if the value of date or x-date exceeds clock
   *                               skew.
   */
    parseRequest: function (request, options) {
      ...
      if (!options) {
        ...
        if (options.headers === undefined) {
          ...
          options.headers.forEach(function (j) {
            if (typeof (j) !== 'string')
              throw new TypeError('options.headers must be an array' +
                                  ' of strings');
          });
        }
      }
    ...
    if (!parsed.scheme || parsed.scheme !== 'Token')
      throw new InvalidHeaderError('scheme was not "Token"');
    ...
    options.headers.forEach(function (j) {
      ...
      if (parsed.params.headers.indexOf(j) < 0)
        throw new MissingHeaderError(j + ' was not a signed header');
    });
