 ...
 *              ExposedSDCError      sdc-client req; expose body.errors,
 *                                   restCode and statusCode; the restCode is
 *                                   that of the cause error, e.g.
 *                                   'ValidationFailedError' from VMAPI.
 *
...
function ExposedSDCError(cause, message) {
    ...
    if (body.errors) {
        body.errors.forEach(function (e) {
            if (e.message) {
                errMsgs.push(format('"%s" (%s) %s',
                    e.field, e.code, e.message));
            } else {
                errMsgs.push(format('"%s" (%s)', e.field, e.code));
            }
        });
