...
function getVPCQuotaForUser(ufdsClient, dataCenterName, accountUuid,
    options, cb) {
    assert.object(ufdsClient, 'ufdsClient');
    assert.string(dataCenterName, 'dataCenterName');
    assert.object(accountUuid, 'accountUuid');
    assert.object(options, 'options');
    assert.object(options.log, 'options.log');
    assert.func(cb, 'cb');

    modConfig.getAccountDcConfigFromUFDS(ufdsClient, accountUuid,
        dataCenterName, {
            log: options.log
    }, function onGetDcLocalConfig(getDcLocalConfigErr, conf) {
        if (getDcLocalConfigErr) {
        ...
        if (!conf) {
            cb(new Error('Could not get user VPC quota'));
            return;
        }

        // No quota set on the user implies we use the default
        // A quota of '0' means no quota.
        if (conf.vpcquota) {
            cb(null, {quota: conf.vpcquota});
            return;
        }
...
module.exports = {
    ...
    getVPCQuotaForUser: getVPCQuotaForUser
};
