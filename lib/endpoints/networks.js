...
var VPC_FIELDS = ['description', 'name', 'cidr', 'uuid'];
var VPC_NETWORK_FIELDS = ['description', 'vpc', 'gateway',
    'name', 'provision_end_ip', 'provision_start_ip',
    'resolvers', 'routes', 'subnet', 'uuid'];
...
/*
 * Similarly, return an error if VPCs aren't enabled in this datacenter
 */
function ensureVPCEnabled(req, res, next) {
    if (!req.config.vpc_enabled) {
        return next(new restify.NotImplementedError(
            'VPC not enabled for this datacenter'));
    }
...
// Note here "net" can be a network, fabric network, vpc network, or
// network_pool from NAPI
function translateNetwork(net) {
    ...
    if (net.fabric) {
        ...
        VPC_NETWORK_FIELDS.forEach(function (p) {
            if (p === 'uuid') {
                return;
            }
...
function listVPCs(req, res, next) {
    ...
    return req.sdc.napi.listVPCs(req.account.uuid, params, reqOpts(req),
        function onList(err, vpcs) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function createVPC(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.CreateVPC, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }

    params.fields = VPC_FIELDS;
    params.owner_uuid = req.account.uuid;

    modNetworks.getVPCQuotaForUser(req.sdc.ufds,
        req.config.datacenter_name, req.account, {
        log: req.log
    }, function _afterGetConf(getVPCQuotaErr, vpcQuota) {
        if (getVPCQuotaErr) {
            next(getVPCQuotaErr);
            return;
        }
        ...
        if (vpcQuota) {
            assert.number(vpcQuota.quota);
            params.quota = vpcQuota.quota;
        }

        req.log.info({
            vpcQuota: vpcQuota.quota
        }, 'User VPC quota');

        req.sdc.napi.createVPC(params, reqOpts(req),
            function onCreate(err, vpc) {
                if (err) {
                    next(translateErr(err));
                    return;
                }
...
function getVPC(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.GetVPC, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.getVPC(params.vpc_id, params, reqOpts(req),
        function onGet(err, vpc) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function updateVPC(req, res, next) {
    assert.ok(req.account);
    assert.ok(req.sdc.napi);
    ...
    try {
        params = schemaValidate(schemas.UpdateVPC, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.updateVPC(vpcId, params, reqOpts(req),
        function onUpdate(err, vpc) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function deleteVPC(req, res, next) {
    assert.ok(req.account);
    assert.ok(req.sdc.napi);
    ...
    try {
        params = schemaValidate(schemas.DeleteVPC, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.deleteVPC(params.vpc_id, params, reqOpts(req),
        function onDel(err) {
            if (err) {
                next(translateErr(err));
                return;
...
function listVPCNetworks(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.ListVPCNetworks, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.listVPCNetworks(params.vpc_id, {}, reqOpts(req),
        function onList(err, networks) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function createVPCNetwork(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.CreateVPCNetwork, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    params.fields = VPC_NETWORK_FIELDS;
    params.owner_uuid = req.account.uuid;

    req.sdc.napi.createVPCNetwork(vpcId, params, reqOpts(req),
        function onCreate(err, network) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function getVPCNetwork(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.GetVPCNetwork, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.getVPCNetwork(params.vpc_id, params.id,
        { fields: FABRIC_NETWORK_FIELDS }, reqOpts(req),
        function onGet(err, network) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function updateVPCNetwork(req, res, next) {
    ...
    try {
        params = schemaValidate(schemas.UpdateVPCNetwork, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.updateVPCNetwork(vpcId, id, params,
        function onUpdate(err, network) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function deleteVPCNetwork(req, res, next) {
    assert.ok(req.account);
    assert.ok(req.sdc.napi);
    ...
    try {
        params = schemaValidate(schemas.DeleteVPCNetwork, req);
    } catch (schemaErr) {
        next(schemaErr);
        return;
    }
    ...
    req.sdc.napi.deleteVPCNetwork(params.vpc_id, params.id, params,
        reqOpts(req), function onDel(err) {
            if (err) {
                next(translateErr(err));
                return;
            }
...
function mountNetworks(server, before, pre) {
    ...
    server.get({
        ...
        name: 'ListVPCs',
        version: [ '9.13.0']
    }, before, ensureVPCEnabled, pre, listVPCs);
    ...
    server.post({
        ...
        name: 'CreateVPC',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, createVPC);

    server.get({
        path: '/:account/vpc/:vpc_id',
        name: 'GetVPC',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, getVPC);
    ...
    server.put({
        ...
        name: 'UpdateVPC',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, updateVPC);

    server.del({
        path: '/:account/vpc/:vpc_id',
        name: 'DeleteVPC',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, deleteVPC);

    // --- VPC Networks

    server.get({
        path: '/:account/vpc/:vpc_id/networks',
        name: 'ListVPCNetworks',
        version: [ '9.13.0']
    }, before, ensureVPCEnabled, pre, listVPCNetworks);

    server.head({
        path: '/:account/vpc/:vpc_id/networks',
        name: 'HeadVPCNetworks',
        version: [ '9.13.0']
    }, before, ensureVPCEnabled, pre, listVPCNetworks);
    ...
    server.post({
        ...
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, createVPCNetwork);

    server.get({
        path: '/:account/vpc/:vpc_id/networks/:id',
        name: 'GetVPCNetwork',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, getVPCNetwork);
    ...
    server.put({
        path: '/:account/vpc/:vpc_id/networks/:id',
        name: 'UpdateVPCNetwork',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, updateVPCNetwork);

    server.del({
        path: '/:account/vpc/:vpc_id/networks/:id',
        name: 'DeleteVPCNetwork',
        version: ['9.13.0']
    }, before, ensureVPCEnabled, pre, deleteVPCNetwork);
