...
function listNetworks(req, res, next) {
    ...
    function resolveSuffix(n, cb) {
    var account = req.account.uuid;
    var nets = [n.id]
    req.sdc.cns.getSuffixesForVM(account, nets, function (err, suf) {
            if (err) {
                req.log.warn('failed to retrieve DNS suffixes from CNS' +
                    ' REST API');
                cb (null, n);
            } else {
                n.suffixes = suf.suffixes;
                cb(null, n);
            }
        });
    };
