...
var MachineHasNoVNCError = errors.MachineHasNoVNCError;
...
function connectVNC(req, res, next) {
    ...
    if (vm.brand !== 'kvm' && vm.brand !== 'bhyve') {
        next(new MachineHasNoVNCError(vm.brand));
        return;
    }
    if (vm.state !== 'running') {
    ...
    if (!res.claimUpgrade) {
        var msg = 'VNC connect endpoint is a websocket and must be upgraded';
...
VNCConnectionFSM.prototype.state_reject = function state_rejectsock() {
...
VNCConnectionFSM.prototype.state_error = function state_error() {
...
VNCConnectionFSM.prototype.state_closed = function state_closed() {
    if (this.socket) {
        this.socket.destroy();
    }
    this.socket = null;
