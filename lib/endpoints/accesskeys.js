 ...
 *
 * Accepts either the full JSON object or a shorthand
 * array of {bucket, level} entries. Returns the
 * canonical JSON string or null if invalid.
 *
 * Time complexity:  O(n) where n = permissions count
 * Space complexity: O(1) beyond the parsed object
 *
 * @param {string|Array|Object} input - Scope parameter
...
function validateScope(input) {
    ...
    if (typeof input === 'string') {
        try {
            scope = JSON.parse(input);
        } catch (e) {
            return {
                valid: false,
                scope: null,
                error: 'scope: invalid JSON'
            };
        }
    } else {
        scope = input;
    ...
    /*
     * Accept shorthand array: [{bucket, level}, ...]
     * and wrap it in the canonical envelope.
     */
    if (Array.isArray(scope)) {
        scope = { version: 1, permissions: scope };
    }
    ...
    if (!scope || typeof scope !== 'object') {
        return {
            valid: false,
            scope: null,
            error: 'scope: must be an object or array'
        };
    }

    if (scope.version !== undefined && scope.version !== 1) {
        return {
            valid: false,
            scope: null,
...
function create(req, res, next) {
    ...
    /*
     * Per-bucket access key scoping. The scope parameter
     * accepts either the full envelope:
     *   {"version":1,"permissions":[{bucket,level},...]}
     * or the shorthand array:
     *   [{bucket,level},...]
     *
     * An empty/null scope means unrestricted (no scope).
     */
    if (req.params.scope) {
