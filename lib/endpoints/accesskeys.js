        ...
        vasync.waterfall([
            ...
            function createKey(_, cb) {
                ufds.addAccessKey(user, account, params,
                    ...
                    /*
                     * Fire-and-forget: push key to mahi Redis for immediate
                     * availability (bypasses replicator delay). Best-effort —
                     * replicator is the authoritative fallback.
                     *
                     * Push ALL keys (not just scoped) so that newly created
                     * keys are usable immediately without waiting for
                     * UFDS→Redis replication.
                     */
                    if (req.sdc.mahi) {
...
function del(req, res, next) {
    ...
    try {
        ufds.deleteAccessKey(user, req.params.accesskeyid, account,
            ...
            /*
             * Revoke key from mahi Redis before responding. Deletion is
             * security-sensitive: the key must not remain usable in the cache
             * after the client receives 204. Applies to all keys (not just
             * scoped) because we don't have scope info at delete time without
             * an extra UFDS read. Harmless no-op for unscoped keys.
             */
            if (req.sdc.mahi) {
                    ...
                    function (revokeErr) {
                        if (revokeErr) {
                            log.warn({err: revokeErr,
                                accesskeyid:
                                    req.params.accesskeyid
                            }, 'cache-revoke best-effort failed');
                        }
...
function update(req, res, next) {
    ...
    try {
        ufds.updateAccessKey(user, account, params,
            ...
            /*
             * Capture secret before translateAccessKey strips it. Fire
             * cache-push when scope changes OR when status changes on a key
             * that has a scope (so deactivation of a scoped key propagates to
             * Redis immediately).
             *
             * Guard: if UFDS did not return the secret (some LDAP stores strip
             * sensitive attrs on update), skip the cache-push to avoid storing
             * a broken entry. The replicator will catch up.
             */
            ...
            if (account) {
                res.header('Location',
                    sprintf('/%s/users/%s/accesskeys/%s', login, user,
                        encodeURIComponent(accesskey.accesskeyid)));
            } else {
                res.header('Location',
