        ...
        vasync.waterfall([
            ...
            function createKey(_, cb) {
                ufds.addAccessKey(user, account, params,
                    ...
                    /*
                     * Fire-and-forget: push scoped key to mahi
                     * Redis for immediate availability (bypasses
                     * replicator delay).  Best-effort — replicator
                     * is the authoritative fallback.
                     */
                    if (req.sdc.mahi && params.accesskeyscope) {
                        req.sdc.mahi.cachePush({
                            accesskeyid: accesskey.accesskeyid,
                            accesskeysecret: accesskeysecret,
                            ownerUuid: req.account.uuid,
                            status: 'Active',
                            scope: params.accesskeyscope
                        }, function (pushErr) {
                            if (pushErr) {
...
function del(req, res, next) {
    ...
    try {
        ufds.deleteAccessKey(user, req.params.accesskeyid, account,
            ...
            /*
             * Fire-and-forget: revoke key from mahi Redis
             * immediately.  Applies to all keys (not just
             * scoped) because we don't have scope info at
             * delete time without an extra UFDS read.
             * Harmless no-op for unscoped keys.
             */
            if (req.sdc.mahi) {
...
function update(req, res, next) {
    ...
    try {
        ufds.updateAccessKey(user, account, params,
            ...
            /*
             * Capture secret before translateAccessKey strips
             * it.  Fire cache-push when scope changes OR when
             * status changes on a key that has a scope (so
             * deactivation of a scoped key propagates to
             * Redis immediately).
             */
