...
function authorize(req, res, next) {
    ...
    function getRoles(_, cb) {
        ...
        // User default roles must be included into the request, given we may
        // need to use them in order to role-tag any resource created by the
        // user, despite of the user assuming or not any non-default role:
        caller.defaultRoles(function (err, defaultRoles) {
            ...
            // If the caller provided a list of roles to operate on a resource
            // as, we look them up and append them to the activeRoles.
            if (requestedRoles && requestedRoles.length) {
                return caller.roles(function (err2, roles) {
                    if (err2) {
                        return cb(err2);
                    }
                    ...
                    for (var i = 0; i !== requestedRoles.length; i++) {
                        ...
                        if (role) {
                            ...
                            var msg = '"' + role + '" not in user roles';
