...
var httpTokenSignature = require('./tokensig');
...
function tokenAuth(req, res, next) {
    ...
    var keyId;
    var log = req.log;
    var authheader = httpTokenSignature.parseRequest(req) 
    var sig = authheader.signature;
    var token = JSON.parse(decodeURIComponent(authheader.params.token));
    var ufds = req.sdc.ufds;
    var keyapi = req.sdc.keyapi;

    if (! token)
      return next(new BadRequestError('No Auth Token'));

    try {
        keyId = authheader.keyId.split('/');
    } catch (e) {
        log.info('Error parsing authorization header: %s', e.stack);
        return next(new BadRequestError(INVALID_AUTHZ));
    }
    ...
    if (!keyId || keyId.length !== 4 || keyId[2] !== 'keys')
        return next(new InvalidCredentialsError(INVALID_KEY, keyId));
    ...
    return ufds.getUser(keyId[1], function (err, account) {
        ...
        return account.getKey(keyId[3], function (err2, key) {
            ...
            log.debug('authSig using key %s => %s', keyId, key.pkcs);
            try {
                if (!httpSignature.verifySignature(authheader, key.pkcs)) {
                    log.debug('authSig::verify FAIL: %j, key=%s',
                              sig, key.standard);
                    return next(new InvalidCredentialsError(INVALID_SIG));
                }
            ...
            var detokenizer = http.request(options, function (tokres) {
              tokres.setEncoding('utf8');
              tokres.on('data', function (chunk) {
                data = JSON.parse(chunk);
                if ( data.developer == developer ) {
                  req.caller = data.account;
                  req.username = data.account.login;
                  return next();               
                } else {
                  return next(new InvalidCredentialsError(INVALID_DEV));
                }
              });
            });

            detokenizer.on('end', function () {
                var user = JSON.parse(data);
                if (user.developer != developer) {
                  /*
                   *  compromised token, being used by an unauthorized
                   * developer
                   */
                  return next(new InvalidCredentialsError(INVALID_DEV));
                }
                req.caller = user.account;
                req.username = user.account.login;
                return next();
            });
            detokenizer.on('error', function(err) {
              return next(new BadRequestError("Could not contact KeyAPI"));
            }); 

            detokenizer.write(JSON.stringify(token));
            detokenizer.end();
        });
    });
}
