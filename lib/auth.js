...
function tokenAuth(req, res, next) {
    ...
    var ufds = req.sdc.ufds;
    var keyapi = "http://" + req.sdc.keyapi.host + ":" + req.sdc.keyapi.port;
        ...
        return account.getKey(keyId[3], function (err2, key) {
            ...
            try {
                if (!httpSignature.verifySignature(authheader, key.pkcs)) {
                    log.debug('authSig::verify FAIL: %j, key=%s',
                              sig, key.standard);
                    console.log("EBADSIG: KEY: " + key + " SIG: " + sig);
                    return next(new InvalidCredentialsError(INVALID_SIG));
                }
            } catch (e) {
                log.debug('Error calling http_signature: ' + e.stack);
            ...
            var detokenizer = restify.createJsonClient({ url: keyapi});

            detokenizer.post('/detoken', token, function ( clienterr, clientreq, clientres, clientobj ) {
              if ( clientobj.developer != developer )
