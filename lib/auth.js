...
function tokenAuth(req, res, next) {
    ...
    function setCreds(err, obj) {
        ...
        for (var i = 0; i < perms.length; i += 1) {
            ...
            if (currPermUrl === req.url) {
                authorized = true;
                break;
            }

            if (req.url.search(currPermUrl) === 0 &&
                currPerm.length === 2 &&
                req.url.charAt(currPermUrl.length) === '/') {
...
function loadAccount(req, res, next) {
    ...
    if (req.params.account === 'my') {
        if (req.subuser) {
            req.params.account = req.caller.account;
        } else {
...
function authorize(req, res, next) {
    ...
    if ((account.uuid !== caller.uuid) &&
            ((typeof(caller.isAdmin) === 'function') && !caller.isAdmin())) {
        if (req.subuser && req.accountMgmt) {
            return req.caller.defaultRoles(function (err, activeRoles) {
                ...
                return req.sdc.mahi.getUser(req.caller.login,
                    ...
                    // PENDING: Need to review if we can go ahead with just a
                    // single IP argument or may need to difference between IPs
                    // by nic tag (private, public, ...).
                    if (req.machine && req.machine.ips) {
                        authOpts.conditions.ip = req.machine.ips;
                    }
                    // It is important to document that in order to use tags in
                    // policy rules, the "::string" type name must be appended
                    // to the tag name: `tag_$tagName::string=$tagValue`:
                    if (req.machine && req.machine.tags) {
                        Object.keys(req.machine.tags).forEach(function (t) {
                            authOpts.conditions['tag_' + t] =
                                req.machine.tags[t];
                        });
                    }
