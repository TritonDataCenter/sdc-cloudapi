...
function authorize(req, res, next) {
    ...
    if ((account.uuid !== caller.uuid) && !caller.isAdmin()) {
        if (req.sdc.mahi && req.subuser && req.config.bleeding_edge_features &&
            ...
            return req.caller.defaultRoles(function (err, activeRoles) {
                ...
                return req.sdc.mahi.getUser(req.caller.login,
                    ...
                    // We may need this for later use if we are creating new
                    // resources and the sub-user didn't specify any role.tag:
