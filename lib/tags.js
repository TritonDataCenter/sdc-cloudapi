...
function replace(req, res, next) {
    ...
    vmapi.setMetadata('tags', tags, {
        ...
        if (err) {
            return next(errors.vmapiErrorWrap(err,
    ...
    return vmapi.getMetadata('tags', t, {
        ...
        if (err) {
            if (err.statusCode === 404) {
                ...
                next(errors.vmapiErrorWrap(err, 'error getting tag'));
            }
        }
...
function delAll(req, res, next) {
    ...
    vmapi.deleteAllMetadata('tags', {
        ...
        if (err) {
            next(errors.vmapiErrorWrap(err, 'error deleting all tags'));
            return;
        }
        log.debug('DELETE %s -> ok', req.path());
        res.send(204);
