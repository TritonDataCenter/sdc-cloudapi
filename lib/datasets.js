...
function loadImages(req, cb) {
    ...
    // we hit the cache if no params were provided
    ...
    return cache.get(req, cacheKey, canUseCache, function (err, cachedImgs) {
        // we treat err here as if it's a cache miss
        if (!err && cachedImgs) {
            return cb(null, cachedImgs);
        }
        ...
        return req.sdc.imgapi.listImages(opts, {
            ...
            if (canUseCache) {
                var lifetime = req.config.redis.max_images_lifetime;  // in sec
                cache.set(req, cacheKey, lifetime, imgs);
            }
...
function loadDisabledImages(req, cb) {
    if (/\/machines/.test(req.url) &&
        ...
        var cacheKey = getCacheKey(req, true);

        return cache.get(req, cacheKey, true, function (err, cachedImgs) {
            // we treat err here as if it's a cache miss
            if (!err && cachedImgs) {
                return cb(null, cachedImgs);
            }
...
function create(req, res, next) {
    ...
    return req.sdc.imgapi.createImageFromVm(data, createOpts, req.account.uuid,
        ...
        cache.invalidate(req, getCacheKey(req));
...
function getCacheKey(req, disabledImages) {
    var key = 'images_' + req.account.uuid;
