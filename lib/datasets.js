...
function translate(req, dataset) {
    ...
    if (dataset.urn && !/\/images/.test(req.url)) {
        obj.urn = dataset.urn;
    }
...
function loadImages(req, cb) {
    ...
    return req.sdc.imgapi.listImages(opts, function (err, imgs) {
        if (err) {
            return cb(err);
        }
        return cb(null, imgs);
...
function create(req, res, next) {
    ...
    var manifestAttributes = [
        'uuid', 'owner', 'description',
        'homepage', 'disabled', 'public',
        'acl', 'tags'
    ];
    ...
    return req.sdc.imgapi.createFromVm(data, {
        vm_uuid: vm_uuid
    }, req.account.uuid, function (err, job, result) {
        if (err) {
            return next(err);
        }
...
function mountCreateImages(server, before) {
    server.post({
        path: '/:account/images',
...
module.exports = {
    load: load,
    mount: mount,
    mountCreateImages: mountCreateImages,
    loadImage: loadImage
};
