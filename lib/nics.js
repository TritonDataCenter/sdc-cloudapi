 ...
 * that there isn't already another NIC on that VM with that network -- if there
 * is, we 302 redirect to that NIC. We also need to check that the server has
 * an appropriate nic tag, otherwise adding that network to the VM will not
 * work.
 *
 ...
 * of the NIC by looking at the 'status' attribute.
 *
...
function addNic(req, res, next) {
    ...
    log.debug({ tag: network.nic_tag, server: serverUuid }, '5');
        req.sdc.napi.listNics({
            ...
            if (nics.length === 0) {
                msg = 'Server does not have NIC tag: ' + network.nic_tag;
    ...
    return vasync.pipeline({
        'funcs': [
            getNetwork, checkNicDoesntExist, checkServerHasTag, addToNapi,
            addToVm, writeResult
        ]
    }, function (err) {
        if (!err || err === 'done') {
        ...
        return removeFromNapi(null, function () {
            // we ignore any error from removeFromNapi(), since the earlier
            // err takes priority
