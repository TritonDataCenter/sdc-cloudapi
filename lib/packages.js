...
// opts - k/v pairs intended to build a search filter.
function searchPackages(opts, req, cb) {
    ...
    req.sdc.papi.list(filter, function (err, pkgs) {
        if (err) {
            return cb(err);
        }
...
function loadPackages(req, res, next) {
    ...
    if (packageUUID && UUID_RE.test(packageUUID)) {
        return req.sdc.papi.get(req.params['package'], function (err, pkg) {
            if (err) {
                return next(err);
            }
            req.pkg = pkg;
    ...
    return searchPackages({
        ...
        if (_p) {
            // If matching by *name*, then prefer *active* packages. If there
            // is more than one active package with the same name, then which
            // wins is not specified.
            var inactiveCandidates = [];
            pkg = req.packages.filter(function (p) {
                ...
                if (_p === p.name) {
                    if (p.active !== 'false') {
                        ...
                        inactiveCandidates.push(p);
                    }
                }

                return undefined;
            });
            if (!pkg && inactiveCandidates.length > 0) {
                pkg = inactiveCandidates[0];
            }
        } else {
            pkg = req.packages.filter(function (p) {
