...
function create(req, res, next) {
    ...
    return ufds.addPolicy(id, entry, function (err, policy) {
        ...
        if (req.headers['role-tag']) {
            ...
            return resources.saveResource(req, function (err3, resource) {
                if (err3) {
                    req.log.error({err: err3},
                        'Error saving role-tags when creating policy');
                }
                return sendResponse();
            });
        } else {
...
function update(req, res, next) {
    ...
    return ufds.modifyPolicy(id, req.params.policy, params,
        ...
        if (req.headers['role-tag']) {
            return resources.saveResource(req, function (e3, resource) {
                if (e3) {
                    req.log.error({err: e3},
                        'Error saving role-tags when updating policy');
                }
                return sendResponse();
            });
        } else {
...
function del(req, res, next) {
    ...
    return ufds.deletePolicy(id, req.params.policy, function (err) {
        ...
        return resources.deleteResource(req, function (err2) {
            if (err2) {
                req.log.error({err: err2},
                    'Error deleting role-tags when deleting policy');
            }
